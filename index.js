...
// [2] تعريف الواجهة (ABI) للتفاعل مع العقود
const MARKET_ABI = [
...
// [4] دالة حفظ السجلات (نظام التوثيق)
...
// [5] دالة جلب السعر الحقيقي من البلوكشين
async function getRealPrice(address) {
    try {
        ...
        console.error("[-] خطأ في قراءة السعر من البلوكشين.");
        return null;
    }
}

// [6] دالة تنفيذ الصفقة الحقيقية
async function executeRealTrade(address, side, price) {
    try {
        const contract = new ethers.Contract(address, MARKET_ABI, wallet);
        console.log(`[📡] جاري تنفيذ أمر ${side} على الشبكة...`);
        ...
        console.log(`[🎉] صفقة مؤكدة على البلوكشين!`);
    } catch (e) {
...
async function startEngine() {
    console.log("=== بدء محرك التداول الحقيقي (Active) ===");
    while (true) {
