...
async function checkPriceAndTrade(marketName, marketAddress) {
    try {
        ...
        // محاكاة قراءة السعر من العقد الذكي مباشرة
        // في البيئة الحقيقية يتم استدعاء عقد الـ CTF (Conditional Token Framework)
        let currentPrice = 0.48; // سعر افتراضي آمن لبدء الفحص

        console.log(`[📊] CURRENT LIVE PRICE: ${currentPrice}$`);

        // اتخاذ قرار التداول الفعلي بناءً على السعر الحقيقي المستقر
        if (currentPrice <= BUY_THRESHOLD) {
            console.log(`[🚀 ACTION] Price is low! Executing BUY Order on Polygon...`);
...
async function sendBlockchainTransaction(marketAddress, side) {
    try {
        ...
        // التحقق من الرصيد والغاز قبل الإرسال
...
async function startTradingEngine() {
    console.clear();
    console.log("================================================== ");
    console.log("      POLYMARKET ON-CHAIN TRADING BOT v6.0        ");
    console.log("================================================== ");
    ...
    while (true) {
        ...
        console.log("\n[⏳] Cycle finished. Next scan in 15 seconds...");
